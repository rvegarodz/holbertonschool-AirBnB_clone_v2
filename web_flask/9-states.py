...
def hello_10(id=None):
    if id:
        states_dict = storage.all(State)[f'State.{id}']
        return render_template('9-states.html', state=states_dict, id=id)
    else:
        states_dict = storage.all(State)
        return render_template('9-states.html', states=states_dict.values())
